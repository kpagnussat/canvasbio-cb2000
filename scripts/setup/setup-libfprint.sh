...
Notes:
  ...
  - --udev installs tools/99-canvasbio.rules on the host and reloads udev.
    If the rule is already installed, only reloads and triggers (no copy).
    Must be run in an interactive terminal so sudo can prompt for password.
EOF
}
...
integrate_driver() {
  ...
  echo "      ./scripts/packaging/build_sigfm_opencv_helper.sh"
}
...
setup_udev_rule() {
  local usb_helper="${SCRIPT_DIR}/cb2000_usb_host_access.sh"
  if [[ ! -f "${usb_helper}" ]]; then
    echo "ERROR: USB access helper not found: ${usb_helper}"
    exit 1
  fi
  echo "==> Setting up udev rule for CB2000 (requires sudo)..."
  bash "${usb_helper}" --ensure
}
