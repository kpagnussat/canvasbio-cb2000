...
Actions:
  prepare-host     install/update the CB2000 udev rule on the host
...
prepare_host() {
  bash "${PROJECT_ROOT}/scripts/setup/cb2000_usb_host_access.sh" --ensure
}

prepare_runtime() {
  ensure_container
  run_in_container "bash scripts/setup/setup-libfprint.sh --install-deps --clone --integrate --build"
  run_in_container "bash scripts/packaging/build_sigfm_opencv_helper.sh"
}
...
build_deb() {
  ensure_container
  run_in_container "bash scripts/packaging/build_libfprint_deb_ubuntu.sh --in-container"
}
