...
VERIFY_GATE_SUMMARY_SCRIPT="${PROJECT_ROOT}/scripts/cb2000_verify_gate_summary.py"
VERIFY_METRICS_SCRIPT="${PROJECT_ROOT}/scripts/cb2000_verify_metrics.py"
VERIFY_BATCH_SUMMARY_SCRIPT="${PROJECT_ROOT}/scripts/cb2000_verify_batch_summary.py"
...
emit_verify_gate_summary() {
  if [[ ! -f "${VERIFY_GATE_SUMMARY_SCRIPT}" ]]; then
    echo "WARN: gate summary script not found: ${VERIFY_GATE_SUMMARY_SCRIPT}"
    return 0
  fi
  ...
  python3 "${VERIFY_GATE_SUMMARY_SCRIPT}" --run "${RUN_DIR}" | tee "${LOG_DIR}/verify_gate_summary.log" || true
}
...
emit_verify_metrics() {
  ...
  if [[ ! -f "${VERIFY_METRICS_SCRIPT}" ]]; then
    echo "WARN: verify metrics script not found: ${VERIFY_METRICS_SCRIPT}"
    return 0
  fi
  ...
  echo
  echo "== Verify Metrics =="
  python3 "${VERIFY_METRICS_SCRIPT}" --run "${RUN_DIR}" --exclude-last "${exclude_last}" \
...
emit_verify_batch_summary() {
  ...
  if [[ ! -f "${VERIFY_BATCH_SUMMARY_SCRIPT}" ]]; then
    echo "WARN: verify batch summary script not found: ${VERIFY_BATCH_SUMMARY_SCRIPT}"
    return 0
  fi
  ...
  cmd=(
    python3 "${VERIFY_BATCH_SUMMARY_SCRIPT}"
    --latest "${latest_count}"
