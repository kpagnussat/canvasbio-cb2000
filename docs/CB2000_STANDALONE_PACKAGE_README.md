...
fingerprint reader, implementing Match-on-Host with a mosaic-first matcher and
gallery fallback.
...
| Metric | Result |
|--------|--------|
| Validation status | functional in controlled lab testing, not broad statistical validation |
| Enrollment stages | 15 (lift-and-shift) |
| Template format | current template stores raw enrollment images plus mosaic data |
| Runtime dependency | `libcb2000_sigfm_opencv.so` required for the full mosaic path |
...
Controlled lab evidence in the project history was strong enough to publish a
working snapshot, but this does **not** yet mean broad statistical validation
across many users, machines, or desktop environments. This public tree should
be read as a working snapshot, not as a fully supported rollout.
...
The orchestrator saves configuration in `.cb2000_env` and
...
│   │   └── cb2000_usb_host_access.sh       # Check and fix CB2000 USB device permissions
│   ├── packaging/
...
│   ├── metrics/
│   │   └── analyze.sh                      # All analysis: run-report, gate-summary, metrics, battery, ...
...
For public GitHub Releases, the packaging flow also stages normalized attachment
names using the shape `<distro>_libfprint-canvasbio-cb2000.<extension>`.
...
1. **Capture and preprocessing**: The driver captures raw 80×64 grayscale
   frames. In the current default optimized path, it builds a SIGFM-clean frame
   from background subtraction plus min/max normalization. The OpenCV helper
   applies its own CLAHE + normalization internally when the helper path is
   used.
...
4. **Template packing**: The current template stores both:
   - the 15 raw enrollment captures
   - the aggregated mosaic keypoints/descriptors

5. **Verification**: The driver first tries the helper-backed
   mosaic matcher. If that path is unavailable or inconclusive, it falls back
   to gallery-style matching against the stored enrollment images. If the probe
   is detected as blank or clearly weak, the driver performs one silent retry
   before returning `NO MATCH`.
...
- `src/cb2000_sigfm_opencv_helper.cpp`
  ...
  - it is compiled separately into `build/libcb2000_sigfm_opencv.so`
    by `scripts/packaging/build_sigfm_opencv_helper.sh`

So the libfprint Meson integration only needs to register the main driver C file.
The OpenCV helper is a sidecar runtime component used by the current mosaic
path.
...
```bash
bash ./scripts/setup/cb2000_usb_host_access.sh --ensure
```
...
- **The OpenCV helper is a required runtime dependency in the current
  snapshot**: the current mosaic path depends on
  `libcb2000_sigfm_opencv.so` being built and available.
